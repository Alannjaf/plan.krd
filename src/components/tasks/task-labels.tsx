...
import { useLabels, queryKeys as labelQueryKeys } from "@/lib/query/queries/labels";
...
export function TaskLabels({
  ...
  const handleToggleLabel = async (label: Label) => {
    ...
    const optimisticLabels = isAssigned
      ? labels.filter((l) => l.label_id !== label.id)
      : [...labels, newTaskLabel];

    // Optimistically update caches BEFORE server call
    queryClient.setQueryData<TaskWithRelations>(taskQueryKeys.task(task.id), (old) =>
      old ? { ...old, labels: optimisticLabels } : old
    );
    queryClient.setQueryData<TaskWithRelations[]>(taskQueryKeys.tasksByBoard(boardId), (old) =>
      old?.map((t) => (t.id === task.id ? { ...t, labels: optimisticLabels } : t))
    );
  ...
  const handleCreateLabel = async () => {
    if (!newLabelName.trim() || readOnly) return;

    const result = await createLabel(boardId, newLabelName.trim(), newLabelColor);
    if (result.success && result.label) {
      onChanged();

      // Create optimistic label data for new label
      const newTaskLabel = {
        id: `temp-${Date.now()}`,
        task_id: task.id,
      ...
      const optimisticLabels = [...labels, newTaskLabel];
      ...
      // Invalidate labels query to refetch the updated list
      queryClient.invalidateQueries({ queryKey: labelQueryKeys.labels(boardId) });
    }
    setNewLabelName("");
    setIsCreating(false);
  };
